...
"""
Script to calculate Dissagregated Red List Index from input CSV.
"""
...
def main() -> int:
    ...
    try:
        df_group = GroupDataFrameProcessor(input_file).df

        # Join the dissagregation DataFrame on 'sis_taxon_id'
        dissagregation_df = pl.read_csv(
            "./tests/fixtures/species_dissagregation_list.csv"
        )
        joined_df = df_group.join(dissagregation_df, on="sis_taxon_id", how="inner")
        ...
        for row in unique_filters.iter_rows(named=True):
            ...
            # Select rows matching the current filter_type and filter
            df = joined_df.filter(
                (pl.col("filter_type") == current_filter_type)
                & (pl.col("filter") == current_filter)
            )

            logging.info(
                f"Processing: filter_type={current_filter_type}, filter={current_filter}"
            )
            ...
            joined_output_file = f"{current_filter_type}_{current_filter}_{output_file}"

            rli_df.write_csv(joined_output_file)
            logging.info(f"Saved results to: {joined_output_file}")
            if args.plot:
                plot = Plot(rli_df)
                plot.global_rli(joined_output_file.replace(".csv", ".png"))
                logging.info(f"Saved plot to: {joined_output_file.replace('.csv', '.png')}")
        return 0
